...
module.exports = function(base) {
  ...
  return replace(reCacheBreak, function(match, resource) {
    ...
    // If we're not dealing with an external file and the file exists, use the
    // files last modified time as the cache breaker
    if(!reExternal.test(resource)) {
      ...
      if(fs.existsSync(fullPath)) {
        qs = fs.statSync(fullPath).mtime.getTime();
      }
    }
